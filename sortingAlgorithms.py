...
def CountSort(A,attr,n,pos,orig_arr):
    maxA=max(A)
    minA=min(A)
    k=maxA-minA+1
    B=[0]*len(orig_arr)
    C=[0]*10
    for i in range(len(A)):
        C[(getattr(orig_arr[i],attr)//pos)%10]+=1
    for i in range(1,10):
        C[i]=C[i]+C[i-1]
    for i in range(len(A)-1,-1,-1):
       
        B[C[((getattr(orig_arr[i],attr)//pos)%10)-1]-4]=getattr(orig_arr[i],attr)
        C[(getattr(orig_arr[i],attr)//pos)%10]-=1
    for i in range(len(A)):
        orig_arr[i]=B[i]
    print(orig_arr)
...
def RadixSort(A,attr,start,end):
    ...
    #for i in range(1,0,maxA%i):
        ...
        C=[0]*10
        for i in range(len(A)):
            C[(getattr(A[i],attr)//pos)%10]+=1
        for i in range(1,10):
            C[i]=C[i]+C[i-1]
        for i in range(len(A)-1,-1,-1):
        
            B[C[((getattr(A[i],attr)//pos)%10)-1]-4]=getattr(A[i],attr)
            C[(getattr(A[i],attr)//pos)%10]-=1
        for i in range(len(A)):
            A[i]=B[i]
